class Links {
  ...
  async existTwitterLink() {
    const twitterLink = await $('a[href="https://twitter.com/saucelabs"]');
    return twitterLink.isExisting();
  }

  async existFacebookLink() {
    const facebookLink = await $(
      'a[href="https://www.facebook.com/saucelabs"]',
    );
    return facebookLink.isExisting();
  }

  async existLinkedInLink() {
    const linkedinLink = await $(
      'a[href="https://www.linkedin.com/company/sauce-labs/"]',
    );
    return linkedinLink.isExisting();
  }
